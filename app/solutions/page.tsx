      ...
      <main className="page-shell pb-20 pt-28">
        <section className="mx-auto max-w-6xl px-5 md:px-8">
          <div className="max-w-3xl">
            <p className="mb-4 text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Solutions</p>
            <AnimatedTitle className="text-4xl font-semibold tracking-[-0.06em] text-slate-900 dark:text-slate-100 md:text-6xl">
              Technology systems designed for <span className="brand-script text-[3.3rem] font-light tracking-[-0.02em] text-slate-900 dark:text-slate-100 md:text-[4.8rem]">startup</span> momentum.
            </AnimatedTitle>
        ...
        <section className="mx-auto mt-16 max-w-6xl px-5 md:px-8">
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
            {offerings.map((item) => (
              <div key={item.title} className="soft-card rounded-3xl bg-white p-6 dark:bg-slate-900">
                <span className="material-symbols-outlined mb-5 block text-3xl text-slate-900 dark:text-slate-100">{item.icon}</span>
                <h2 className="mb-3 text-xl font-semibold text-slate-900 dark:text-slate-100">{item.title}</h2>
        ...
        <section className="mx-auto mt-20 max-w-6xl px-5 md:px-8">
          <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr]">
            <div className="soft-card rounded-[28px] bg-white p-8 dark:bg-slate-900">
              <p className="mb-4 text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">What you get</p>
              <h2 className="text-3xl font-semibold tracking-[-0.05em] text-slate-900 dark:text-slate-100 md:text-4xl">
                A <span className="brand-script text-[3rem] font-light leading-none text-slate-900 dark:text-slate-100 md:text-[4rem]">delivery</span> system built around real business priorities.
              </h2>
              <p className="mt-5 text-base leading-7 text-slate-600 dark:text-slate-300">
                We align engineering, design, and operational <span className="brand-script text-[2rem] font-light leading-none text-slate-900 dark:text-slate-100">decision-making</span> so your tooling keeps pace with customer demand, compliance requirements, and internal growth.
              </p>
            </div>
              ...
              <ul className="space-y-4 text-base text-slate-700 dark:text-slate-200">
                {deliverables.map((item) => (
                  <li key={item} className="flex items-start gap-3">
                    <span className="material-symbols-outlined mt-0.5 text-xl text-slate-900 dark:text-slate-100">check_circle</span>
                    <span>{item}</span>
                  </li>
                ))}
              </ul>
        ...
        <section className="mx-auto mt-20 max-w-5xl px-5 md:px-8">
          <div className="rounded-[28px] bg-slate-900 p-8 text-white md:p-10">
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              <Link href="/contact" className="soft-button inline-flex rounded-full bg-white px-5 py-3 text-sm font-medium text-slate-900 transition hover:bg-slate-100">
                Book a discovery call
              </Link>
              <Link href="/pricing" className="soft-button-secondary inline-flex rounded-full bg-slate-800 px-5 py-3 text-sm font-medium text-white transition hover:bg-slate-700">
                View pricing
              </Link>
            </div>
          </div>
