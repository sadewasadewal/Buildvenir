      ...
      <main className="page-shell pb-20 pt-28">
        <section className="mx-auto max-w-6xl px-5 md:px-8">
          <div className="max-w-3xl">
            <p className="mb-4 text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400">Pricing</p>
            <AnimatedTitle className="text-4xl font-semibold tracking-[-0.06em] text-slate-900 md:text-6xl">
              Simple <span className="brand-script text-[3.3rem] font-light tracking-[-0.02em] text-slate-900 md:text-[4.8rem]">packages</span>. Scalable support.
            </AnimatedTitle>
            <p className="mt-6 text-lg leading-8 text-slate-600">
              Flexible engagement models for <span className="brand-script text-[2rem] font-light leading-none text-slate-900">startup</span> teams, growth-stage operators, and leadership seeking a more dependable technical foundation.
            </p>
          </div>
        </section>
        ...
        <section className="mx-auto mt-16 max-w-6xl px-5 md:px-8">
          <div className="grid gap-6 lg:grid-cols-3">
            {plans.map((plan) => (
              <div key={plan.name} className={`rounded-[28px] border p-7 ${plan.highlight ? 'border-slate-900 bg-slate-900 text-white shadow-[0_20px_60px_rgba(15,23,42,0.18)]' : 'border-slate-200 bg-white text-slate-900'}`}>
                <p className={`text-[11px] font-semibold uppercase tracking-[0.2em] ${plan.highlight ? 'text-slate-300' : 'text-slate-400'}`}>{plan.name}</p>
                <div className="mt-5 text-4xl font-semibold tracking-[-0.06em]">{plan.price}</div>
                <p className={`mt-4 text-base leading-7 ${plan.highlight ? 'text-slate-300' : 'text-slate-600'}`}>{plan.blurb}</p>

                <ul className={`mt-6 space-y-3 ${plan.highlight ? 'text-slate-200' : 'text-slate-700'}`}>
                ...
                <div className="mt-8">
                  <Link href="/contact" className={`inline-flex rounded-full px-5 py-3 text-sm font-medium transition ${plan.highlight ? 'bg-white text-slate-900 hover:bg-slate-100' : 'bg-slate-900 text-white hover:bg-slate-700'}`}>
        ...
        <section className="mx-auto mt-20 max-w-5xl px-5 md:px-8">
          <div className="rounded-[28px] border border-slate-200 bg-white p-8 md:p-10">
            <p className="mb-6 text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-400">FAQs</p>
